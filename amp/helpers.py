...
def prep(X, str_fcn=lambda x: x.strip().lower(), col_fcn=lambda x: x.strip().lower().replace(' ','_').replace('-','_'), idx=True, col=True, **kwargs):
    def fcn(df):
        ...
        if col:
            df = g(df)
        if idx and not isinstance(df.index, pd.RangeIndex):
            I = g(df[[]].reset_index())
            df = df.set_index(pd.MultiIndex.from_frame(I))
        return df
    return wrap2(fcn, X)
setmeth_pd(prep)
...
def dump(path, obj, **kwargs):
    ...
    if p.suffix == '.parquet':
        obj.to_parquet(p, engine="pyarrow", coerce_timestamps="us", allow_truncated_timestamps=True, **kwargs)
